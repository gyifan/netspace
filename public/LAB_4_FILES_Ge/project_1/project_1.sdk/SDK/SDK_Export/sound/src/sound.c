...
int main () {
	...
	// Set private timer prescaler to 256
	...
	while(1){
		for(i=0; i<TOTAL_NODE; i++){
			...
			do{
				Status = checkGlobalTimer(xTimer);
				...
				// Wait until private timer goes to 0
				do {
					privateCnt = XScuTimer_GetCounterValue(PrivateTimerPtr);
				} while(0 != privateCnt);

				// Toggle out sound output
				XGpioPs_WritePin(&psGpioInstancePtr,SOUND_LEFT, nextVal);
				XGpioPs_WritePin(&psGpioInstancePtr,SOUND_RIGHT, nextVal);
				nextVal = (nextVal == 1) ? 0 : 1; // update nextVal
